...
function bitcoin() {
  docker exec -ti fuse_bitcoind bitcoin-cli -chain=regtest -rpcuser=regtest -rpcpassword=regtest -rpcwait "$@"
}

function lndserver() {
  docker exec -ti fuse_lndserver lncli --tlscertpath /root/.lnd/tls.cert --macaroonpath /root/.lnd/data/chain/bitcoin/regtest/admin.macaroon "$@"
}

function lndclient() {
  docker exec -ti fuse_lnd lncli --tlscertpath /root/.lnd/tls.cert --macaroonpath /root/.lnd/data/chain/bitcoin/regtest/admin.macaroon "$@"
}
...
function setup() {
  ...
  lndserver openchannel --node_key $LNDCLIENT --connect fuse_lnd:9735 --local_amt 16000000
  mine 6
